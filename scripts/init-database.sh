...
case $ENVIRONMENT in
    dev)
        ...
        CONTAINER_NAME=together-mysql-dev
        ;;
    staging)
        DB_PORT=3308
        DB_NAME=together_staging
        CONTAINER_NAME=together-mysql-staging
        ;;
    prod)
        DB_PORT=3309
        DB_NAME=together_prod
        CONTAINER_NAME=together-mysql-prod
        ;;
    *)
...
# 检查表数量
echo "检查数据库表..."
TABLE_COUNT=$(docker exec $CONTAINER_NAME mysql -u$DB_USER -p$DB_PASSWORD -e "SELECT COUNT(*) as cnt FROM information_schema.TABLES WHERE TABLE_SCHEMA = '$DB_NAME';" 2>/dev/null | tail -1)
...
if [ "$TABLE_COUNT" -eq "0" ]; then
    ...
    if [ "$ENVIRONMENT" != "dev" ]; then
        ...
        # 检查 dev 数据库是否有表
        DEV_TABLE_COUNT=$(docker exec together-mysql-dev mysql -u$DB_USER -p$DB_PASSWORD -e "SELECT COUNT(*) as cnt FROM information_schema.TABLES WHERE TABLE_SCHEMA = 'together_dev';" 2>/dev/null | tail -1 || echo "0")

        if [ "$DEV_TABLE_COUNT" -gt "0" ]; then
            echo "从 dev 环境导出表结构..."
            docker exec together-mysql-dev mysqldump -u$DB_USER -p$DB_PASSWORD --no-data --skip-add-drop-table together_dev 2>/dev/null > /tmp/schema_export.sql

            echo "导入表结构到 $ENVIRONMENT 环境..."
            docker exec -i $CONTAINER_NAME mysql -u$DB_USER -p$DB_PASSWORD $DB_NAME < /tmp/schema_export.sql 2>/dev/null

            # 验证导入
            NEW_TABLE_COUNT=$(docker exec $CONTAINER_NAME mysql -u$DB_USER -p$DB_PASSWORD -e "SELECT COUNT(*) as cnt FROM information_schema.TABLES WHERE TABLE_SCHEMA = '$DB_NAME';" 2>/dev/null | tail -1)
    ...
    echo ""
    echo "现有表列表:"
    docker exec $CONTAINER_NAME mysql -u$DB_USER -p$DB_PASSWORD -e "SHOW TABLES FROM $DB_NAME;" 2>/dev/null | tail -n +2 | head -20
fi
